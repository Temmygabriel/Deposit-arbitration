...
const CONTRACT_ADDRESS = "0xf0A21af7270a3b991627550940B5Eb8DcAAcc902";
const PRIVATE_KEY = process.env.NEXT_PUBLIC_FUNDED_KEY as string;
...
async function writeContract(fn: string, args: (string | number | boolean | bigint)[]): Promise<boolean> {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      console.error(`writeContract ${fn} attempt ${attempt} failed:`, msg);
...
export default function Home() {
  ...
  const checkStatus = useCallback(async (id: number, navigate = true) => {
    ...
    setCaseStatus(cs);
    setStatusChecking(false);
    if (cs === "final") { setScreen("final_verdict"); }
    else if (cs === "round1_complete") { setScreen("verdict"); }
    else if (cs === "appeal_filed") { setScreen("appeal_pending"); }
    else if (cs === "ready_verdict") { setCheckResult("ready"); if (navigate) setScreen("status"); }
    else { setCheckResult("not_yet"); if (navigate) setScreen("status"); }
  }, []);
  ...
  const handleCreateCase = async () => {
    ...
    const ok = await writeContract("create_case", [hostName, guestName, propertyAddress, `${depositAmount} ${currency}`, agreementTerms]);
  ...
  const handleLoadToRespond = async () => {
    ...
    if (myClaimFiled) { setCaseStatus(getCaseStatus(state)); setScreen("status"); }
    else { setScreen("respond_claim"); }
  };
  ...
  const handleHomeLoad = async () => {
    ...
    if (isNaN(id) || id < 1) { setError("Please enter a valid case ID"); return; }
    setError(""); setLoading(true); setLoadingMsg("Loading case...");
    setCaseId(id); setLoading(false);
    await checkStatus(id);
  };

  const handleRequestVerdict = async () => {
    if (!caseId) return;
    setLoading(true); setLoadingMsg("5 AI validators are reading both sides... this takes 30–60 seconds");
    ...
    setCaseData(state); setLoading(false); setScreen("verdict");
  };
  ...
  const handleAcceptVerdict = async () => {
    ...
    if (!ok) { setError("Transaction failed. Please try again."); setLoading(false); return; }
    const state = await readCase(caseId);
    setCaseData(state); setLoading(false); setScreen("final_verdict");
  };
  ...
  const handleFileAppeal = async () => {
    ...
    if (!ok) { setError("Transaction failed. Please try again."); setLoading(false); return; }
    const state = await readCase(caseId);
    setCaseData(state); setLoading(false); setScreen("appeal_pending");
  };

  const handleResolveAppeal = async () => {
    if (!caseId) return;
    setLoading(true); setLoadingMsg("Appellate panel reviewing previous verdict... this takes 30–60 seconds");
    ...
    <main className="poh-main">
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        :root {
          --ink0: #0a0a0a; --ink1: #111111; --ink2: #1a1a1a; --ink3: #242424;
          --ink4: #2e2e2e; --ink5: #3a3a3a; --muted1: #888; --muted2: #aaa;
          --text: #f0ece0; --text2: #c8c4b4; --red: #c0392b; --red2: #e74c3c;
          --gold: #d4a843; --gold2: #f0c060; --green: #27ae60; --green2: #2ecc71;
          --blue: #2980b9; --blue2: #3498db; --orange: #e67e22; --orange2: #f39c12;
          --r: 10px; --r2: 16px;
        }
          ...
          <div className="poh-home">
            ...
            <div className="poh-appeals-banner">
              ...
              <div>
                ...
                <div className="poh-appeals-desc">If you disagree with the Round 1 verdict, file an appeal. A fresh panel of 5 AI validators re-reads the full case — and must explicitly address why they are upholding or overturning the original ruling. After Round 2, the verdict is locked permanently onchain.</div>
              </div>
            </div>
          ...
          <div className="poh-form-wrap">
            ...
            <div className="poh-role-grid">
              <button className={`poh-role-card poh-role-host ${myRole === "host" ? "poh-role-active-host" : ""}`} onClick={() => { setMyRole("host"); setError(""); }}>
                <span className="poh-role-icon">🏠</span><span className="poh-role-title">I am the Host</span>
              ...
              <button className={`poh-role-card poh-role-guest ${myRole === "guest" ? "poh-role-active-guest" : ""}`} onClick={() => { setMyRole("guest"); setError(""); }}>
                <span className="poh-role-icon">👤</span><span className="poh-role-title">I am the Guest</span>
            ...
            <div className="poh-two-paths">
              <div className="poh-path-card">
                ...
                <button className="poh-btn-red poh-btn-full" onClick={() => { if (!myRole) { setError("Please select your role first"); return; } setError(""); setScreen("create"); }}>Start New Dispute →</button>
              </div>
              ...
              <div className="poh-path-card">
                ...
                <div style={{marginBottom:"0.75rem"}}><input className="poh-input" placeholder="Enter case ID e.g. 5" value={loadId} onChange={e=>{setLoadId(e.target.value); setError("");}} /></div>
            ...
            <div className="poh-card">
              ...
              <div className="poh-field">
                ...
                <div className="poh-amount-row">
                  <select className="poh-currency-select" value={currency} onChange={e=>setCurrency(e.target.value)}>{CURRENCIES.map(c => <option key={c} value={c}>{c}</option>)}</select>
        ...
        {screen === "my_claim" && (
          <div className="poh-form-wrap">
            ...
            <div className="poh-card">
              ...
              <div className="poh-field"><label>Your Claim</label><textarea className="poh-textarea" placeholder={myRole === "host" ? "Describe why you are withholding the caution fee. Be specific about what damage or rule violation occurred." : "Describe why the caution fee should be refunded. Explain your stay and checkout condition."} value={myClaim} onChange={e=>setMyClaim(e.target.value)} rows={4} /></div>
              <div className="poh-field"><label>Your Evidence</label><textarea className="poh-textarea" placeholder={myRole === "host" ? "List your evidence — damage photos, repair invoices, inspection reports, WhatsApp messages, etc." : "List your evidence — check-in photos, messages from host, receipts, WhatsApp screenshots, etc."} value={myEvidence} onChange={e=>setMyEvidence(e.target.value)} rows={4} /></div>
              {error && <p className="poh-error">{error}</p>}
        ...
        {screen === "respond_claim" && caseData && (
          <div className="poh-form-wrap">
            ...
            <div className="poh-card">
              ...
              <div className="poh-field"><label>Your Claim</label><textarea className="poh-textarea" placeholder={myRole === "guest" ? "Describe why the caution fee should be refunded. Be specific about your stay and checkout condition." : "Describe why you are withholding the caution fee. Be specific about damage or rule violations."} value={myClaim} onChange={e=>setMyClaim(e.target.value)} rows={4} /></div>
              <div className="poh-field"><label>Your Evidence</label><textarea className="poh-textarea" placeholder={myRole === "guest" ? "List your evidence — check-in photos, messages from host, receipts, WhatsApp screenshots, etc." : "List your evidence — damage photos, repair invoices, inspection reports, messages, etc."} value={myEvidence} onChange={e=>setMyEvidence(e.target.value)} rows={4} /></div>
              {error && <p className="poh-error">{error}</p>}
            ...
            {caseStatus === "waiting_other" && (
              <>
                ...
                <div className="poh-card">
                  ...
                  <div className="poh-share-note"><span className="poh-share-note-icon">💬</span><span>Send via WhatsApp, SMS, or email. They only need the ID number.</span></div>
                  <div className="poh-status-check-block">
                    <p className="poh-status-check-label">Once they&apos;ve responded, press this to check:</p>
                    <button className="poh-btn-red poh-btn-full" disabled={statusChecking} onClick={async () => { setCheckResult(null); if (caseId) await checkStatus(caseId); }}>
                      {statusChecking ? "⏳ Checking the blockchain..." : `Check if ${knownRole ? otherLabel : "Other Party"} Has Responded →`}
                    </button>
                    {checkResult === "not_yet" && <div className="poh-check-result poh-check-waiting"><span>⏳</span><span>{knownRole ? otherLabel : "Other party"} has <strong>not responded yet.</strong> Send them the ID and ask them to file their side.</span></div>}
                    {checkResult === "ready" && <div className="poh-check-result poh-check-ready"><span>✅</span><span>Both sides are in! Scroll down to request the verdict.</span></div>}
                  </div>
                  {error && <p className="poh-error">{error}</p>}
                </div>
              </>
              ...
              <>
                ...
                <div className="poh-card">
                  <div className="poh-ready-banner">
                    ...
                    <div><div className="poh-ready-title">Both sides have filed their claims</div><div className="poh-ready-sub">The AI judges are standing by. This takes 30–60 seconds once requested.</div></div>
                  </div>
                  <div>
                    <p className="poh-validators-label">5 AI validators will evaluate independently:</p>
                    <div className="poh-chips">{["GPT-5.1","Grok-4","Qwen3-235b","Claude Sonnet","+ more"].map(c=><span key={c} className="poh-chip">{c}</span>)}</div>
            ...
            <div className="poh-verdict-screen">
              ...
              <div className="poh-verdict-cards">
                ...
                <div className="poh-vcard poh-consensus-card">
                  ...
                  <div className="poh-chips" style={{marginTop:"1rem"}}>{["GPT-5.1 ✓","Grok-4 ✓","Qwen3-235b ✓","Claude Sonnet ✓","Majority Agree ✓"].map(c=><span key={c} className="poh-chip-agree">{c}</span>)}</div>
                  ...
                  <div className="poh-appeal-actions">
                    {!myRole && (
                      <>
                        ...
                        <button className="poh-btn-outline poh-btn-full" onClick={() => { setLoadId(String(caseData.case_id)); setScreen("role_select"); }}>Select My Role →</button>
                      </>
                    )}
                    {myRole && (() => {
                      ...
                      if (iWon) {
                        return (
                          <div style={{background:"rgba(39,174,96,0.06)",border:"1px solid rgba(39,174,96,0.2)",borderRadius:"var(--r)",padding:"1.25rem",display:"flex",flexDirection:"column" as const,gap:"0.75rem",alignItems:"center",textAlign:"center" as const}}>
                            <div style={{fontSize:"2rem"}}>🏆</div>
                            <div style={{fontSize:"1rem",fontWeight:700,color:"var(--green2)"}}>You won this round</div>
                            <div style={{fontSize:"0.82rem",color:"var(--muted2)",lineHeight:1.65,maxWidth:"320px"}}>The {myRole === "host" ? "guest" : "host"} can accept this verdict or file an appeal within their own session. Check back using Case ID <strong style={{color:"var(--text2)"}}>#{caseData.case_id}</strong>.</div>
                            <div style={{display:"flex",alignItems:"center",gap:"8px",background:"rgba(255,255,255,0.04)",border:"1px solid var(--ink4)",borderRadius:"999px",padding:"0.3rem 1rem",fontSize:"0.75rem",color:"var(--muted1)",fontFamily:"monospace"}}>
                        ...
                        return (
                          <>
                            <div style={{background:"rgba(192,57,43,0.06)",border:"1px solid rgba(192,57,43,0.2)",borderRadius:"var(--r)",padding:"0.75rem 1rem",fontSize:"0.82rem",color:"#f5a0a0",lineHeight:1.65}}>
                            ...
                            <button className="poh-btn-red poh-btn-full" onClick={() => setScreen("appeal")}>🔁 I Disagree — File an Appeal (Round 2)</button>
                            <div className="poh-appeal-divider"><span>or</span></div>
                            <button className="poh-btn-green poh-btn-full" onClick={handleAcceptVerdict}>✓ I Accept This Verdict — Lock It Final</button>
                          </>
                        );
                      }
                    })()}
          ...
          <div className="poh-form-wrap">
            ...
            <div className="poh-card">
              ...
              <div className="poh-field">
                ...
                <textarea className="poh-textarea" placeholder="Explain why the Round 1 verdict was wrong. What did the judges miss? What evidence was not properly considered? Be specific — the appellate panel will directly respond to this." value={appealReason} onChange={e=>setAppealReason(e.target.value)} rows={5} />
              </div>
              {error && <p className="poh-error">{error}</p>}
        ...
        {screen === "appeal_pending" && caseData && (
          <div className="poh-form-wrap">
            ...
            <div className="poh-card">
              ...
              {error && <p className="poh-error">{error}</p>}
              <button className="poh-btn-gold poh-btn-full poh-btn-gavel" onClick={handleResolveAppeal}>⚖️ Summon Appellate Panel — Final Round</button>
            </div>
          </div>
        )}
              ...
              <div className="poh-verdict-cards">
                ...
                {wasAppealed && (
                  <div className={`poh-vcard poh-appeal-outcome-card ${wasOverturned ? "poh-overturned" : "poh-upheld"}`}>
                    ...
                    {wasOverturned ? <div className="poh-outcome-result-overturned">🔄 Round 1 Verdict Overturned</div> : <div className="poh-outcome-result-upheld">✅ Round 1 Verdict Upheld</div>}
                ...
                {wasAppealed && (
                  <div className="poh-vcard">
                    ...
                    <div className="poh-details-grid" style={{marginBottom:"0.5rem"}}><span className="poh-dl">Round 1 winner</span><span className="poh-dv">{caseData.round1_winner === "guest" ? "Guest" : "Host"}</span></div>
                ...
                <div className="poh-vcard poh-consensus-card">
                  <h3>🔗 Onchain Consensus</h3>
                  <p>This final verdict was reached by {wasAppealed ? "two independent panels of" : ""} 5 AI validators on GenLayer&apos;s Bradbury testnet — transparent, auditable, and tamper-proof. This verdict cannot be changed.</p>
                  <div className="poh-chips" style={{marginTop:"1rem"}}>{["GPT-5.1 ✓","Grok-4 ✓","Qwen3-235b ✓","Claude Sonnet ✓","Majority Agree ✓"].map(c=><span key={c} className="poh-chip-agree">{c}</span>)}</div>
                  <div className="poh-contract-ref">Contract: <span className="poh-mono">{CONTRACT_ADDRESS.slice(0,10)}...{CONTRACT_ADDRESS.slice(-6)}</span></div>
                </div>
